...
class GalaxyViewController: UIViewController {
    ...
    override func didMove(toParent parent: UIViewController?) {
        ...
        if parent == nil {
            delegate?.notifyStateMachine(source: self, .Back, galaxy!, coordinator: coordinator!)
        }
    }
...
extension GalaxyViewController: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        delegate?.notifyStateMachine(source: self, .Next, skyObject, coordinator: coordinator!)
    }
}
